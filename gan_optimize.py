...
x = data.iloc[:, :num_columns].values
y = data.iloc[:, num_columns].values
...
def objective(trial):
    ...
    # Generator hyperparameters for GRU
    gen_params = {
        'gru1_units': trial.suggest_int('g_gru1_units', 512, 2048),
        'gru2_units': trial.suggest_int('g_gru2_units', 256, 1024),
        'gru3_units': trial.suggest_int('g_gru3_units', 128, 512),
        'linear1_units': trial.suggest_int('g_linear1_units', 64, 256),
        'linear2_units': trial.suggest_int('g_linear2_units', 32, 128),
        'dropout_rate': trial.suggest_float('g_dropout_rate', 0.1, 0.5),
        'learning_rate': trial.suggest_float('learning_rate_g', 1e-5, 1e-2, log=True)
    }

    # Discriminator hyperparameters for CNN
    disc_params = {
        ...
        'linear2_units': trial.suggest_int('d_linear2_units', 128, 512),
        'dropout': trial.suggest_float('d_dropout', 0.1, 0.5),
        'learning_rate': trial.suggest_float('learning_rate_d', 1e-5, 1e-2, log=True)
    }

    # Train VAE
    model_vae = VAE(input_dim=num_columns, hidden_dims=vae_params['hidden_dims'], 
    ...
    class Generator(nn.Module):
        ...
        def forward(self, x):
            h0_1 = torch.zeros(1, x.size(0), gen_params['gru1_units']).to(x.device)
            out, _ = self.gru_1(x, h0_1)
            out = self.dropout(out)

            h0_2 = torch.zeros(1, x.size(0), gen_params['gru2_units']).to(x.device)
            out, _ = self.gru_2(out, h0_2)
            out = self.dropout(out)

            h0_3 = torch.zeros(1, x.size(0), gen_params['gru3_units']).to(x.device)
            out, _ = self.gru_3(out, h0_3)
            out = self.dropout(out)
    ...
    # Updated CNN Discriminator
    class Discriminator(nn.Module):
        def __init__(self):
            super().__init__()
            self.conv1 = nn.Conv1d(4, disc_params['conv1_filters'], kernel_size=3, stride=1, padding='same')
            self.conv2 = nn.Conv1d(disc_params['conv1_filters'], disc_params['conv2_filters'], kernel_size=3, stride=1, padding='same')
            self.conv3 = nn.Conv1d(disc_params['conv2_filters'], disc_params['conv3_filters'], kernel_size=3, stride=1, padding='same')
            self.linear1 = nn.Linear(disc_params['conv3_filters'], disc_params['linear1_units'])
            self.linear2 = nn.Linear(disc_params['linear1_units'], disc_params['linear2_units'])
            self.linear3 = nn.Linear(disc_params['linear2_units'], 1)
    ...
    # GAN Training Loop (inside `objective`)
    ...
    for epoch in range(num_epochs):
        for (x, y) in trainDataloader:
            fake_data = modelG(x)
            fake_data = torch.cat([y[:, :3, :], fake_data.unsqueeze(1)], axis=1)

            dis_real_output = modelD(y.permute(0, 2, 1))  # Permute for Conv1D
            real_labels = torch.ones_like(dis_real_output)
            lossD_real = criterion(dis_real_output, real_labels)

            dis_fake_output = modelD(fake_data.permute(0, 2, 1))  # Permute for Conv1D
            ...
            output_fake = modelD(fake_data.permute(0, 2, 1))  # Permute for Conv1D
